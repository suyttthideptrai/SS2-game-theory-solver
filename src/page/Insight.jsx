...
export default function InsightPage() {
  ...
  const handleExportToExcel = async () => {
    ...
    const numberOfCores = appData.insights.params.distributedCoreParam === 'all'
        ? 'All available cores'
        : appData.insights.params.distributedCoreParam + ' cores';
    const sheet3 = XLSX.utils.aoa_to_sheet([
      ...
      [
        ...
        appData.insights.params.generationParam],
      [
        'Optimization execution max time (milliseconds)',
        appData.insights.params.maxTimeParam],
    ]);
    ...
    const sheet4 = XLSX.utils.aoa_to_sheet([
      [
        ...
        appData.insights?.data?.computerSpecs?.osFamily || 'unknown'],
      [
        'Operating System Manufacturer',
        appData.insights?.data?.computerSpecs?.osManufacturer || 'unknown'],
      [
        'Operating System Version',
        appData.insights?.data?.computerSpecs?.osVersion || 'unknown'],
      ['CPU Name', appData.insights?.data?.computerSpecs?.cpuName || 'unknown'],
      [
        'CPU Physical Cores',
        appData.insights?.data?.computerSpecs?.cpuPhysicalCores || 'unknown'],
      [
        'CPU Logical Cores',
        appData.insights?.data?.computerSpecs?.cpuLogicalCores || 'unknown'],
      [
        'Total Memory',
        appData.insights?.data?.computerSpecs?.totalMemory || 'unknown'],
    ]);
  ...
  return (
      <div className="insight-page">
        ...
        <div className="fitness-table">
          ...
          <p className="figure-description">Comparison of Fitness Values across
            different algorithms</p>

        </div>
        <div className="runtime-graph">
          <Line class="graph" data={graphData} option={graphOptions}/>
          <p className="figure-description">Comparison of runtime (in seconds)
            across various algorithms</p>
